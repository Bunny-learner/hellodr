import React, { useState, useEffect } from 'react';
import { useForm } from 'react-hook-form';
import '../../css/patientsignup.css'; // UPDATED: Corrected import path
import { Link } from 'react-router-dom';
import { useNavigate } from 'react-router-dom';
import { LinearProgress, Button } from '@mui/material';
import toast, { Toaster } from 'react-hot-toast';
const API = import.meta.env.VITE_API_URL;
export default function Patientsignup() {
  const { register, handleSubmit, formState: { errors } } = useForm();
  const [passwordVisible, setPasswordVisible] = useState(false);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const msg = params.get('alert');
    if (msg) toast.error(msg);
  }, [location.search]);

  const navigate = useNavigate();
  const back = () => {
    navigate('/');
  }
  const togglePasswordVisibility = () => {
    setPasswordVisible(!passwordVisible);
  };

  const onSubmit = async (data) => {
    const ele = document.querySelector('.ps-form-body');
    const btn = ele.getElementsByTagName('button')[0];
    btn.disabled = true;
    btn.backgroundColor = "darkgreen";
    setLoading(true);

    await fetch(`${API}/patient/signdata`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        name: data.username,
        email: data.email,
        dob: data.dob,
        gender: data.gender,
        phone: data.phone,
        password: data.password
      })
    })
      .then((res) => {
        if (res.status === 201) {
          setLoading(false);
          btn.disabled = false;
          navigate('/patient/login?alert=Signup failed, This Mail Has already been Registered');
        }

        if (res.status === 202) {
          setLoading(false);
          navigate('/patient/home?alert=Signup was Successful');
        }
      })
      .catch((err) => console.log(err));
  };

  return (
    <div className="ps-main">
      {loading && (
        <LinearProgress
          color="primary"
          className="ps-progress"
        />
      )}
      <div className="ps-registration-page">
        <Toaster position="top-left" toastOptions={{ className: "ps-my-toast" }} reverseOrder={false} />

        {/* This is the Back Button you added */}
        <div className='ps-back'>
          <svg onClick={back} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="#044141ff" className="ps-size-6">
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 15 3 9m0 0 6-6M3 9h12a6 6 0 0 1 0 12h-3" />
          </svg>
        </div>

        <div className="ps-registration-card">
          <div className="ps-form-header">
            {/* SVG Logo updated to use a solid fill color instead of gradient */}
             <svg width="70" height="70" viewBox="0 0 70 70" fill="none" xmlns="http://www.w3.org/2000/svg">
<g clip-path="url(#clip0_495_3740)">
<path d="M64.9511 16.4914H53.5086V5.04886C53.5086 2.26596 51.2426 0 48.4597 0H21.5364C18.7535 0 16.4875 2.26596 16.4875 5.04886V16.4914H5.04886C2.26596 16.4914 0 18.7535 0 21.5403V48.4636C0 51.2465 2.26596 53.5125 5.04886 53.5125H16.4914V64.955C16.4914 67.7379 18.7535 70.0039 21.5403 70.0039H48.4636C51.2465 70.0039 53.5125 67.7379 53.5125 64.955V53.5125H64.955C67.7379 53.5125 70.0039 51.2465 70.0039 48.4636V21.5403C70 18.7535 67.734 16.4914 64.9511 16.4914ZM64.613 48.1255H53.5086V27.2576H48.1255V64.613H21.8745V53.5086H42.7385V48.1255H5.38312V21.8745H16.4875V42.7385H21.8706V5.38312H48.1177V16.4875H27.2615V21.8706H64.6169V48.1255H64.613Z" fill="url(#paint0_linear_495_3740)"/>
</g>
<defs>
<linearGradient id="paint0_linear_495_3740" x1="-1.69238e-07" y1="3.58994" x2="76.3966" y2="18.4545" gradientUnits="userSpaceOnUse">
<stop stop-color="#0EBE7E"/>
<stop offset="1" stop-color="#07D9AD"/>
</linearGradient>
<clipPath id="clip0_495_3740">
<rect width="70" height="70" fill="white"/>
</clipPath>
</defs>
</svg>
            <h1>Patient Registration</h1>
          </div>
          <div className="ps-social-login-buttons">
            <a className="ps-social-btn" href={`${API}/auth/google?type=signup`}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" /><path d="M1 1h22v22H1z" fill="none" /></svg> Google </a>
            <a className="ps-social-btn" href={`${API}/auth/facebook?type=signup`}> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#1877F2"><path d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.84 3.44 8.87 8 9.8V15H8v-3h2V9.5C10 7.57 11.57 6 13.5 6H16v3h-1.5c-1 0-1.5.5-1.5 1.5V12h3l-.5 3h-2.5v6.8c4.56-.93 8-4.96 8-9.8z" /></svg> Facebook </a>
          </div>
          <form onSubmit={handleSubmit(onSubmit)} className="ps-form-body" noValidate>
            <div className="ps-input-group">
              <svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="ps-size-6">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
              </svg>
              <input
                type="text"
                placeholder="Username"
                className={`ps-form-input ${errors.username ? 'ps-input-error' : ''}`}
                {...register("username", { required: "Username is required" })}
                name="username"
              />
              {errors.username && <p className="ps-error-message">{errors.username.message}</p>}
            </div>

            <div className="ps-input-group">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="ps-size-6">
                <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
              </svg>
              <input
                type="email"
                placeholder="Email"
                name="email"
                className={`ps-form-input ${errors.email ? 'ps-input-error' : ''}`}
                {...register("email", {
                  required: "Email is required",
                  pattern: {
                    value: /^\S+@\S+$/i,
                    message: "Invalid email format"
                  }
                })}
              />
              {errors.email && <p className="ps-error-message">{errors.email.message}</p>}
            </div>

            <div className="ps-input-group">
              <input
                name="password"
                type={passwordVisible ? 'text' : 'password'}
                placeholder="Password"
                className={`ps-form-input ${errors.password ? 'ps-input-error' : ''}`}
                {...register("password", {
                  required: "Password is required",
                  minLength: {
                    value: 8,
                    message: "Password must have at least 8 characters"
                  }
                })}
              />
              <span className="ps-password-toggle-icon" onClick={togglePasswordVisibility}>
                {passwordVisible ? (
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16"><path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0z" /><path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z" /></svg>
                ) : (
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16"><path d="m10.79 12.912-1.614-1.615a3.5 3.5 0 0 1-4.474-4.474l-2.06-2.06C.938 6.278 0 8 0 8s3 5.5 8 5.5a7.029 7.029 0 0 0 2.79-.588zM5.21 3.088A7.028 7.028 0 0 1 8 2.5c5 0 8 5.5 8 5.5s-.939 1.721-2.641 3.238l-2.062-2.062a3.5 3.5 0 0 0-4.474-4.474L5.21 3.089z" /><path d="M5.525 7.646a2.5 2.5 0 0 0 2.829 2.829l-2.83-2.829zm4.95.708-2.829-2.83a2.5 2.5 0 0 1 2.829 2.829zm3.171 6-12-12 .708-.708 12 12-.708.708z" /></svg>
                )}
              </span>
              {errors.password && <p className="ps-error-message">{errors.password.message}</p>}
            </div>

            <div className="ps-input-group">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="ps-size-6">
                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z" />
              </svg>
              <input
                type="tel"
                name="phone_number"
                placeholder="Phone Number"
                className={`ps-form-input ${errors.phone ? 'ps-input-error' : ''}`}
                {...register("phone", {
                  required: "Phone number is required",
                  pattern: {
                    value: /^[0-9]{10}$/,
                    message: "Enter a valid 10-digit number"
                  }
                })}
              />
              {errors.phone && <p className="ps-error-message">{errors.phone.message}</p>}
            </div>

            <div className="ps-input-group">
              <select
                name="gender"
                className={`ps-form-input ${errors.gender ? 'ps-input-error' : ''}`}
                {...register("gender", { required: "Gender is required" })}
              >
                <option value="">Select Gender</option>
                <option value="male">Male</option>
                <option value="female">Female</option>
                <option value="other">Other</option>
              </select>
              {errors.gender && <p className="ps-error-message">{errors.gender.message}</p>}
            </div>

            <div className="ps-input-group">
              <input
                type="date"
                name="dob"
                className={`ps-form-input ${errors.dob ? 'ps-input-error' : ''}`}
                {...register("dob", { required: "Date of birth is required" })}
              />
              {errors.dob && <p className="ps-error-message">{errors.dob.message}</p>}
            </div>

            <button type="submit" className="ps-signup-btn">
              Sign Up
            </button>
          </form>

          <p className="ps-login-prompt">
            Already have an account? <Link to="/patient/login">Login</Link>
          </p>
        </div>
      </div>
    </div>
  );
}