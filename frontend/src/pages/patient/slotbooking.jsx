...
export default function SlotBooking() {
  ...
  // --- MODIFIED ---
  // Now returns the full Date object for sorting
  const getNextDateForDay = (dayName) => {
    ...
    if (dayIndex === -1) return null; // Handle invalid day

    const diff = (dayIndex - today.getDay() + 7) % 7;
    const nextDate = new Date(today);
    nextDate.setDate(today.getDate() + diff);

    // Set to midnight for consistent date-only comparison
    nextDate.setHours(0, 0, 0, 0);

    return nextDate; // Return the full Date object
  };
    ...
    const fetchSlots = async () => {
      ...
      try {
        ...
        (data.timeslots || []).forEach((slot) => {
          ...
          if (!grouped[key]) {
            // --- MODIFIED ---
            // Get Date object (for sorting) and formatted date string (for display)
            const nextDateObj = getNextDateForDay(slot.Day || key);
            if (!nextDateObj) return; // Skip if day name is invalid

            grouped[key] = {
              id: key,
              dayName: slot.Day || key,
              date: formatDate(nextDateObj), // For display
              dateObj: nextDateObj, // For sorting
        ...
        // --- THIS IS THE FIX ---
        // Sort the array by the date object before saving to state
  ...
  // when mode changes (no change to this)
  useEffect(() => {
  ...
  // helper: does there exist any day with online slots? (no change)
  ...
  const goto = () => {
    ...
    const appointmentData = {
      ...
      date: selectedDayData.date, // This will still work as it's the formatted string
              ...
              <div key={period} className="time-period">
                ...
                {modeSlots[period].length > 0 ? (
                  <div className="slots-grid">
                    {[...modeSlots[period]]
                      .sort((a, b) => {
                        // Get the start time (e.g., "6:10" from "6:10 - 6:20")
                        const startTimeA = a.label.split("-")[0].trim();
                        const startTimeB = b.label.split("-")[0].trim();
                      ...
                      .map((slot) => (
                        <button
                          key={slot.label}
                          className={`slot-btn ${
                            selectedSlot?.label === slot.label ? "slot-selected" : ""
                          }`}
                          onClick={() => {
                        ...
                        >
                          ...
                          <div className="slot-info">
                            {mode === "offline" && (
                              <span>
                                {Math.max(0, (slot.limit || 0) - (slot.booked || 0))} left
                              </span>
                            )}
                          </div>
                        </button>
